...
class Breakthrough():
        ...
        if len(self.__Locks) > 0:
            ...
            while not self.__GameOver:
                ...
                while not self.__LockSolved and not self.__GameOver:
                    ...
                    print(self.__CurrentLock.GetLockDetails(self.__Sequence._Cards))
                    ...
                    elif MenuChoice == "P" and not self.__CurrentLock.GetPeek:
                        self.__CurrentLock.SetPeek(True)
                    Counter += 1
                    if self.__CurrentLock.GetLockSolved():
                        self.__Score += (20 - Counter) if 20 - Counter > 0 else 0
                        print ("You got " + str(20 - Counter) if 20 - Counter > 0 else "0" + " bonus points")
                        self.__LockSolved = True
                        self.__ProcessLockSolved()
                    print ("You have " + str(self.__Deck.GetNumberOfCards()) + " left in your deck" )
                    if self.__Credits > 1:
                        if input("Buy a tool? (Y/N) :> ").upper() == "Y":
    ...
    def __PlayCardToSequence(self, CardChoice):
        if self.__Hand.GetCardDescriptionAt(CardChoice - 1)[2] == "m":
            self.__Hand.SetCardToolkit(CardChoice - 1)
    ...
    def __LoadGame(self, FileName):
        try:
            with open(FileName) as f:          
                LineFromFile = f.readline().rstrip()   #8
                self.__Score = int(LineFromFile)
                LineFromFile = f.readline().rstrip()   #P a, F a, P a ; K b ; P c, F b, P a ; K a
                LineFromFile2 = f.readline().rstrip()  #N;Y;N;Y
                self.__SetupLock(LineFromFile, LineFromFile2)  
                LineFromFile = f.readline().rstrip()   #P a 1,F a 16,P b 2,K a 19,P a 4
                self.__SetupCardCollectionFromGameFile(LineFromFile, self.__Hand)
                LineFromFile = f.readline().rstrip()   #K b 20,F c 30,K a 31
                ...
                LineFromFile = f.readline().rstrip()   #P a 7,Dif 34,P a 10,P a 13,K a 25,K b 26...
    ...
    def __GetCardFromDeck(self, CardChoice):
        if self.__Deck.GetNumberOfCards() > 0:
            ...
            if self.__Deck.GetCardDescriptionAt(0) == "Gen":
                CurrentCard = self.__Deck.RemoveCard(self.__Deck.GetCardNumberAt(0))
                print()
                print("Genuis card drawn")
                self.__Discard.AddCard(CurrentCard)
                CurrentCard.Process(self.__CurrentLock)
    ...
    def __GetChoice(self):
        peek = ", (P)eek" if not self.__CurrentLock.GetPeek() else ""
        print()
        Choice = input("(D)iscard inspect, (U)se card" + peek + ":> ").upper()
        return Choice
    ...
    def __AddGeniusCardsToDeck(self):
        if random.randint (0,3) == 0:
            self.__Deck.AddCard(GeniusCard())

    def __AddMultitools(self):
        for tool in ["P", "F", "K"]:
    ...
    def __CreateStandardDeck(self):
        for deck in ["a", "b", "c"]:
            for i in range(5):
                NewCard = ToolCard("P", deck)
                self.__Deck.AddCard(NewCard)
            for i in range (3):
                NewCard = ToolCard("F", deck)
                self.__Deck.AddCard(NewCard)
                NewCard = ToolCard("K", deck)
                self.__Deck.AddCard(NewCard)
        self.__AddMultitools()
...
class Lock():
    ...
    def GetLockDetails(self, Sequence):
        Sequence = [x.GetDescription() for x in Sequence]
        
        LockDetails = "\n" + "CURRENT LOCK" + "\n" + "------------" + "\n"
        for C in self._Challenges:
            if C.GetMet():
                LockDetails += "Challenge met: "
            elif C.GetCondition()[:1] == Sequence[-1:] or C.GetCondition()[:2] == Sequence[-2:]:
...
class ToolCard(Card):
    ...
    def SetCardToolkit(self):
        self._Kit = str(input("Enter which tool kit (a, b, c) to set this multi tool to :> ")).lower()
...
class GeniusCard(Card):
    ...
    def Process(self, CurrentLock):
        if input("(U)se genuis card or (D)iscard it :> ").upper() == "U":
            ChallengesLeft = CurrentLock.GetNumberOfChallenges()
            for i in CurrentLock.GetNumberOfChallenges() - 1:
                ChallengesLeft -= 1 if CurrentLock.GetChallengeMet(i) else 0
            if ChallengesLeft > 1:
                CurrentLock.SetChallengeMet(int(input("Enter 1-" + CurrentLock.GetNumberOfChallenges() + " to specifiy challenge to solve :> "))-1)
            else:
                for i in CurrentLock.GetNumberOfChallenges():
                    CurrentLock.SetChallengeMet(True)
...
class CardCollection():
    ...
    def SetCardToolkit(self, CardNumber):
        self._Cards[CardNumber].SetCardToolkit()        
